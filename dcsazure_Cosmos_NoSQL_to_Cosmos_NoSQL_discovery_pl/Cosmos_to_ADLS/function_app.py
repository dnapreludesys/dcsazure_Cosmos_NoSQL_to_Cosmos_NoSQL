...
class CosmosMetadata:
    ...
    def get_throughput_info(database, container_name: str) -> Dict:
        ...
        try:
            ...
            try:
                ...
                if offer:
                    ...
                    throughput = (offer['content'].get('maximumThroughput')
                                  or offer['content'].get('throughput', DEFAULT_THROUGHPUT))
                    return {
                        'level': 'container',
            ...
            return {'level': 'unknown', 'throughput': DEFAULT_THROUGHPUT, 'is_autoscale': False}
...
class TokenBucketRUManager:
    ...
    def __init__(self, throughput: int, reserve_percent: float = DEFAULT_RESERVE_PERCENT):
        """
...
class StreamingCosmosReader:
    ...
    def __init__(self, container, pk_path: Optional[str], pk_values: Optional[List], 
                 batch_size: int, throughput: int = DEFAULT_THROUGHPUT):
        """
        ...
        self.ru_manager = TokenBucketRUManager(throughput, reserve_percent=DEFAULT_RESERVE_PERCENT)
...
def extract_arrays_iterative_optimized(doc: Dict) -> Tuple[Dict, Dict[str, pd.DataFrame]]:
    """
    ...
    batch_size = ARRAY_PROCESSING_BATCH_SIZE
    current_batch = {}
...
def upload_csv_to_adls(service_client, file_system: str, directory: str, 
                       file_name: str, df: pd.DataFrame, mode: str = 'write', 
                       known_columns: Optional[set] = None) -> set:
    """
...
def validate_cosmos_connection(cosmos_url: str, cosmos_key: str, 
                               cosmos_db: str, cosmos_container: str) -> Tuple:
    """
...
def setup_partition_configuration(params: dict, container) -> Tuple[Optional[str], Optional[List], bool]:
    """
...
def process_document_batch(batch_docs: List[Dict], batch_start: int) -> Tuple[pd.DataFrame, Dict]:
    """
...
def upload_child_tables(service_client, adls_file_system: str, export_dir: str,
                       cosmos_container: str, batch_child_tables: Dict, 
                       child_schemas: Dict, batch_num: int, mode: str,
                       separate_files_per_batch: bool) -> Dict:
    """
...
def process_cosmos_to_adls_activity(params: dict):
    ...
    try:
        ...
        pk_path, pk_values, user_provided_both = setup_partition_configuration(params, container)

        throughput_info = CosmosMetadata.get_throughput_info(database, params["cosmos_container"])
        ...
        for batch_docs in streaming_reader.stream_documents():
            ...
            parent_df, batch_child_tables = process_document_batch(batch_docs, batch_start)
            total_parent_rows += len(parent_df)
            
            # Upload parent table
            if params["separate_files_per_batch"]:
                parent_file_name = f"{params['cosmos_container']}_batch_{batch_num:03d}.csv"
                mode = 'write'
                parent_schema = None
            else:
